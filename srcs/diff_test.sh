...
diff_test()
{
	...
	do
		...
		# Run test - logic for FD vs Standard (FASTER: No leak checker inside loop)
	...
	# --- Optimized Leak Check (Runs ONCE per function) ---
	local valgrind_log="/tmp/leaks_libft_all_$$.log"
	local leak_status=0

	if [ "$(uname)" == "Darwin" ]; then
		# macOS: Run all tests in a loop under leaks tool
		...
		# Linux: Run all tests in a loop under valgrind
	...
	if [ $leak_status -eq 42 ]; then
		leak_found=1
		printf "\n= Memory Leak Detection " >> "${PATH_DEEPTHOUGHT}"/deepthought
		printf "%.s=" $(seq 1 56) >> "${PATH_DEEPTHOUGHT}"/deepthought
		printf "\n" >> "${PATH_DEEPTHOUGHT}"/deepthought
